...
def _load_data(dataset=None, data_path='./data_download/'):
    if(dataset == 'BACE'):
        ...
        task = 'classification'
        target = ["logP"]
        csv_file_path = data_path + "/logP.csv"

    elif (dataset == 'ToxCast'):
        task = 'classification'
        target = [0 for i in range(600)]
        target =[
        ...
        csv_file_path = data_path + "/ToxCast.csv"
...
def _process_csv(csv_file, target, task):
    ...
    smiles_data, labels = [], {}# {} :字典
    for t in target:
        labels[t] = []

    for i, row in tqdm(enumerate(csv_reader)):# https://blog.csdn.net/windmyself/article/details/119413613,tqdm 进度条
        # Skip header
        if i == 0:
            continue
...
def read_smiles(dataset, data_path):
    ...
    return smiles_data, np.array(labels)#np.array(x)：将字典转化为一维数组
