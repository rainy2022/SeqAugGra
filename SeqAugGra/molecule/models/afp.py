...
class GATEConv(MessagePassing):
    def __init__(self, emb_dim, dropout=0.0):
        ...
        self.edge_embedding1 = nn.Embedding(NUM_BOND_TYPE, emb_dim)  # 其他特征怎么体现，直接加进去
        self.edge_embedding2 = nn.Embedding(NUM_BOND_DIRECTION, emb_dim)
        self.edge_embedding3 = nn.Embedding(NUM_STEREO_LIST, emb_dim)
...
class AttentiveFP(nn.Module):
    ...
    """
    def __init__(self, task, emb_dim=300, num_layers=5, num_timesteps=3, drop_ratio=0.5,
        ...
        if self.task == 'classification':
            self.output_dim *= 2
            self.lin2 = nn.Linear(emb_dim + 300, self.output_dim)
    ...
    def forward(self, data):
        ...
        x = self.x_embedding1(x[:, 0]) + self.x_embedding2(x[:, 1]) + self.x_embedding3(x[:, 2]) + self.x_embedding4(
            x[:, 3]) \
        ...
        h_add = torch.concat([out, vec], -1)

        return h_add, self.lin2(h_add)
